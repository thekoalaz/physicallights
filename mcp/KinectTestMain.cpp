...
void main(int argc, char **argv)
{
	...
	std::string command_prefix = "setAttr lights:aiAreaLight_key";
	std::string commandX_prefix = command_prefix + ".translateX ";
	std::string commandZ_prefix = command_prefix + ".translateZ ";
	std::string	commandX;
	std::string	commandZ;
	double angle = 0;
	double radius = 300;
	...
	while(true) {
		...
		commandX = commandX_prefix + std::to_string(x);
		commandZ = commandZ_prefix + std::to_string(z);
		/*if ( readStdIn ) {
			if ( interactive ) {
		...
		if ( verbose )
			fprintf(stderr,"// %s: sending command %s\n",program, commandX.c_str());
			fprintf(stderr,"// %s: sending command %s\n",program, commandZ.c_str());

		//send(fd,command,strlen(command),0);
		send(fd,commandX.c_str(),commandX.length(),0);
		...
		send(fd,commandZ.c_str(),commandZ.length(),0);
		if ( verbose )
			fprintf(stderr,"// %s: awaiting reply...",program);

		red = recv(fd,reply,4096,0);
